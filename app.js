...
function globePoints(){
  ...
  const savedPlaces=[...getFavorites(),selected].filter(place=>Number.isFinite(Number(place?.latitude))&&Number.isFinite(Number(place?.longitude)));
  const locationPoints=savedPlaces.filter((place,index,list)=>list.findIndex(other=>Number(other.latitude)===Number(place.latitude)&&Number(other.longitude)===Number(place.longitude))===index).map(place=>({type:'Localidade',name:place.name||'Localidade escolhida',latitude:Number(place.latitude),longitude:Number(place.longitude),detail:'Localidade selecionada para acompanhamento',color:'#76e7bb'}));
  return [...earthquakePoints,...disasterPoints,...cyclonePoints,...locationPoints];
}
function globe(){
